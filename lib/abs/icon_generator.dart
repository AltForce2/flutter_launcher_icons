...
class IconGeneratorContext {
  ...
  IconGeneratorContext({required this.config, required this.logger, required this.prefixPath, this.flavor});
...
Future<void> generateIconsFor({
  ...
  try {
    final platformList = platforms(
      IconGeneratorContext(config: config, logger: logger, prefixPath: prefixPath, flavor: flavor),
    );
    ...
    for (final platform in platformList) {
      ...
      try {
        ...
        logger.info('• Creating Icons ${platform.platformName}');
