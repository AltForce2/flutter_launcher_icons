...
class Config {
  ...
  const Config({
    ...
    this.windowsConfig,
    this.macOSConfig,
    this.adaptiveIconPath,
    this.adaptiveIconName,
  });

  /// Creates [Config] for given [flavor] and [prefixPath]
  static Config? loadConfigFromFlavor(String flavor, String prefixPath) {
    return _getConfigFromPubspecYaml(prefix: prefixPath, pathToPubspecYamlFile: utils.flavorConfigFile(flavor));
  }
  ...
  static Config? loadConfigFromPath(String filePath, String prefixPath) {
    return _getConfigFromPubspecYaml(prefix: prefixPath, pathToPubspecYamlFile: filePath);
  }

  /// Loads flutter launcher icons config from `pubspec.yaml` file
  static Config? loadConfigFromPubSpec(String prefix) {
    return _getConfigFromPubspecYaml(prefix: prefix, pathToPubspecYamlFile: constants.pubspecFilePath);
  }

  static Config? _getConfigFromPubspecYaml({required String pathToPubspecYamlFile, required String prefix}) {
    ...
    try {
      return yaml.checkedYamlDecode<Config?>(configContent, (Map<dynamic, dynamic>? json) {
        if (json != null) {
  ...
  /// android notification_icon_path
  @JsonKey(name: 'android_notification_icon_path')
  final String? notificationIconPath;

  /// android notification_icon_name
  @JsonKey(name: 'android_notification_icon_name')
  ...
  /// Adaptive icon path
  @JsonKey(name: 'adaptive_icon_path')
  final String? adaptiveIconPath;

  /// Adaptive icon name
  @JsonKey(name: 'adaptive_icon_name')
  final String? adaptiveIconName;
  ...
  bool get hasAndroidAdaptiveConfig => isNeedingNewAndroidIcon && adaptiveIconPath != null && adaptiveIconName != null;
  ...
  /// whether or not there is configuration for notification icons for android
  bool get hasAndroidNotificationConfig {
    return isNeedingNewAndroidIcon && notificationIconPath != null && notificationIconName != null;
  }
  ...
  /// get the notification icon path for Android
  String? getNotificationIconPath() => notificationIconPath;

  /// get the notification icon name for Android
  String? getNotificationIconName() => notificationIconName;
